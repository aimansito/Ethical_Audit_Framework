...
class PDFReportGenerator:
    def __init__(self, host: Host):
        ...
        self.filename = Path(f"REPORT_{host.ip.replace('.','_')}_{self.timestamp}.pdf")
        ...
        if self.host.vulnerabilities:
            ...
            for vuln in self.host.vulnerabilities:
                ...
                story.append(Paragraph(
                    f"<font color='{color[vuln.risk.name]}'>{vuln.name}</font>",
                    styles['Heading2']
                ))
                story.append(Paragraph(vuln.description, styles['Normal']))
                if vuln.recommendations:
                    story.append(Paragraph(f"<i>✅ {vuln.recommendations}</i>", styles['Italic']))
